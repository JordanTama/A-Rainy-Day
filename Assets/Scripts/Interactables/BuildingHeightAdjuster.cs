...
public class BuildingHeightAdjuster : InteractableReceiver
{
    ...
    {
        _bisAtEnd = true;
        _moveTween = transformToMove.DOLocalMove(_endLocalPosition, t).OnComplete(MeshUpdate);
    }
    
    private void MoveToStart(float t)
    {
        _bisAtEnd = false;
        _moveTween = transformToMove.DOLocalMove(_defaultLocalPosition, t).OnComplete(MeshUpdate);
    }
